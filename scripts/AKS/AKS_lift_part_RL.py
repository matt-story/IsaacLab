...
.. code-block:: bash
    ...
    ./isaaclab.sh -p scripts/AKS/AKS_lift_part_RL.py
            ...
            if trunc.any():
                ...
                if idx <= len(generated_poses) - 1:
                    ...
                    if highest_reward > 10:
                        ...
                        print(f"No successful grasps found {no_pick_count} times. Highest reward at {idx} with value {highest_reward} for part at {part_pose_rounded}")
                        no_pick_count += 1
                        if no_pick_count >= 10:
                            no_pick_count = 0
                            np.save(f"{pose_folder}check_part_pose.npy", part_pose_rounded)
                            np.save(f"{pose_folder}success_grasp_poses.npy", np.array([]))
