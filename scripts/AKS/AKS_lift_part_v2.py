...
.. code-block:: bash
    ...
    ./isaaclab.sh -p scripts/AKS/AKS_lift_part_v2.py
...
def main():
    ...
    grasp_poses = grasper_manager.grasp_locations
    grasp_orientations = grasper_manager.grasp_orientations
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            if len(to_reset_list) == env.unwrapped.num_envs:
                ...
                if reward_list[0][1] <= 0.0:
                    no_pick = True

                current_rewards = np.array(reward_list[:5])

                # print(f"Prev rewards: {prev_top_5}")
                # print(f"Curr rewards: {current_rewards}") 
                print(f"Rewards: {reward_list[:5]}")               

                if np.array_equal(current_rewards, prev_top_5):
                    print(f"Converged on top 5 rewards after {counter} attempts, generate new grasp poses.")
                    
                    # Generate new grasp poses
                    success_generation = grasper_manager.generate_grasp_poses()
                    if not success_generation or not grasper_manager.grasp_locations:
                        print("Failed to generate grasp poses or no poses were generated.")
                    else:
                    ...
                    print(f"No successful picks detected, generate new grasp poses.")
                    # Generate new grasp poses
                    success_generation = grasper_manager.generate_grasp_poses()
                    if not success_generation or not grasper_manager.grasp_locations:
                        print("Failed to generate grasp poses or no poses were generated.")
                    ...
                    print(f"Not yet converged, continuing with current grasps. Attempt {counter+1}.")
                    prev_top_5 = current_rewards
                    counter += 1
