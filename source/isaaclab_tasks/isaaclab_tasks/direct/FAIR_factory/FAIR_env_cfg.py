...
from .FAIR_tasks_cfg import ASSET_DIR, FAIRTask, GearMesh, NutThread, PegInsert, Flashlight
...
@configclass
class FAIREnvCfg(DirectRLEnvCfg):
    decimation = 8
    action_space = 6
    ...
    task_name: str = "flashlight"  # peg_insert, gear_mesh, nut_thread
    task: FAIRTask = FAIRTask()
...
@configclass
class FactoryTaskPegInsertCfg(FAIREnvCfg):
...
@configclass
class FactoryTaskGearMeshCfg(FAIREnvCfg):
    task_name = "gear_mesh"
    task = GearMesh()
...
@configclass
class FactoryTaskNutThreadCfg(FAIREnvCfg):
    task_name = "nut_thread"
    task = NutThread()
    episode_length_s = 30.0

@configclass
class FactoryTaskFlashlightCfg(FAIREnvCfg):
