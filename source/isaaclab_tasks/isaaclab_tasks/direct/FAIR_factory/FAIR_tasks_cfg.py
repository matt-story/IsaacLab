...
@configclass
class Flashlight_shell(HeldAssetCfg):
...
@configclass
class kitting_tray(FixedAssetCfg):
    usd_path = assets_folder + "Collected_AKS_picking/assembly_parts/flashlight_kitting_tray.usd"
    diameter = 0.0081
    height = 0.025
    base_height = 0.0

@configclass
class Flashlight(FAIRTask):
    name = "flashlight"
    fixed_asset_cfg = kitting_tray()
    held_asset_cfg = Flashlight_shell()
    asset_size = 36.0
    duration_s = 10.0
    ...
    # tray_rotation = euler_angles_to_quat(np.array([0.0, 0.0, 0.0]))
    ...
    held_asset: ArticulationCfg = ArticulationCfg(
        ...
        init_state=ArticulationCfg.InitialStateCfg(
            pos=(0.0, 0.4, 0.3), rot=(1.0, 0.0, 0.0, 0.0), joint_pos={}, joint_vel={}
        ),
        actuators={},
    )

@configclass
class PegInsert(FAIRTask):
    name = "peg_insert"
